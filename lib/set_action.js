...
function setBallMove(type, t, end_point, reb_point, act_beg) {
  ...
  if(type == PLAIN) {
    ...
    k.push({frame: 0, value: sphere.position});
    k.push({frame: t*30, value: end_point });    
    moveMesh(sphere, k, "position", BABYLON.Animation.ANIMATIONTYPE_VECTOR3, null, null, act_beg);
    }
  } else if(type == CURVE) {
...
function getEndPoint(act) {
  ...
  if(first_act_succ[0] && _.indexOf(['sht', 'sav', 'shu', 'sft', 
                                     'bkh', 'pas','run', 'rnb'], first_act_succ[0].type.substring(0, 3)) != -1)
  ...
  if(first_act_succ[0] && _.indexOf(['dhs', 'rns', 'sld'], first_act_succ[0].type.substring(0, 3)) != -1) {
    ...
    scale = 0.1;
  } else if(first_act_succ[0] && _.indexOf(['bre'], first_act_succ[0].type.substring(0, 3)) != -1) {
    scale = 0.02;
  }
  //console.log(scale);
...
function doAction(a) { 
  ...
  if(a.type.substring(0, 3) == 'pas') {
    ...
    setSave(a, SAVE_FOOT);
  } else if(a.type.substring(0, 3) == 'shu') {
    setSave(a, SAVE_ENDUP);
  ...
  } else if(a.type == 'breaststop') {
    end_point = new BABYLON.Vector3(sphere.position.x, -5, sphere.position.z);
    setSimpleAction(a, 0.5, 0, 10, false, {},function(){setBallMove(PLAIN, 1, end_point, null, a.beg);});
  }
