...
function set_act_obj(val) {
    ...
    if(val.to) { //se val.to non è null (nella save può esserlo)
      ...
      f_pos = val.to.split(",");
      if(val.hd && val.hd=="u") {        
        val.to = field_zone_m[f_pos[0]][f_pos[1]].clone();
        val.to.y = 4;
...
function startActions(ad) {   
  ...
  console.log(action_list[0].to);
  action_list.forEach(set_act_obj); //converte le proprietà delle a. s. da stringhe a tipi opportuni
  console.log(action_list[0].mesh.position);
  console.log(action_list[0].to);
  _.each(act_ini, doAction); //setta per l'esecuzione le a. i.
}

function getActions() {
  //var actions_url = "actionstest8b.txt";
  var actions_url = "https://raw.githubusercontent.com/doctorjay66/digirep/master/actionstest8b.txt";
